    ...
    for i, (data1, data2)  in enumerate(data_loader):
        with torch.no_grad():
            if not "sda" in mode:
                ...
                result1 = model_o(return_loss=False, **data1)
                result2 = model(return_loss=False, **data2)
                result = ensemble_from_logits(result1, result2, mode)
                data = data1
...
def multi_gpu_test_ensemble(model, model_o, data_loader, mode, tmpdir=None, gpu_collect=False):
